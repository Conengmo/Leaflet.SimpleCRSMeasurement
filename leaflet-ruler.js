  ...
  L.Control.Ruler = L.Control.extend({
    ...
    _clicked: function(e) {
      ...
      if(this._clickCount > 0 && !e.latlng.equals(this._clickedPoints[this._clickedPoints.length - 2])){
        ...
        let text = this.options.formatter(this._totalLength);
    ...
    _moving: function(e) {
      if (this._clickedLatLong){
        ...
        let text = this.options.formatter(this._addedLength);
    ...
    _calculateDistance: function() {
      var dx = this._movingLatLong.lng - this._clickedLatLong.lng,
          dy = this._movingLatLong.lat - this._clickedLatLong.lat;
      var distance = Math.sqrt(dx * dx + dy * dy) * this.options.scale;
      this._result = {
        Bearing: 0,
        Distance: distance
      };
